...
3. Dann nmußte ich mit `conda` separat [Pillow](http://cognitiones.kantel-chaos-team.de/programmierung/python/pillow.html) hinzufügen, da `pip` in der Umgebung Pillow nicht installieren konnte.
4. *Last but not least* konnte ich dann mit `pip install p5` auch P5 (Python) mit den restlichen Abhängigkeiten installeren.
