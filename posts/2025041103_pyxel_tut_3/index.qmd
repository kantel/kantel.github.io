...
eine Pyxel-Standalone-App erzeugt und diese dann mit

~~~bash
pyxel app2html your_app.pyxapp
~~~

in eine HTML-Datei gewandelt. Das Ergebnis ist zwar erstaunlich kompakt, ruft aber leider immer noch die Webversion von Pyxel über ein CDN auf.

Denn [Quellcode und alle Assets](https://github.com/kantel/pyxel/tree/main/nightinpark) inklusive der `.pixapp` und der daraus erzeugten HTML-Datei findet Ihr in meinem GitHub-Repositorium, damit Ihr ebenfalls damit experimentieren könnt.

Jetzt bin ich hin- und hergerissen: Soll ich mit Pyxel erst einmal weiter experimentieren oder soll ich zu microStudio zurückkehren? Das Spielen mit Pyxel macht mir Spaß und speziell mit dem Ressourceneditor möchte ich noch einiges anstellen (so gefällt mir die [Default-Farbpalette](http://cognitiones.kantel-chaos-team.de/multimedia/farbpaletten/pyxelpal.html) nicht, ich würde sie gerne entweder durch die [Pico-8 Farbpalette](http://cognitiones.kantel-chaos-team.de/multimedia/farbpaletten/pico8.html) oder die [TIC-80 Farbpalette](http://cognitiones.kantel-chaos-team.de/multimedia/farbpaletten/sweetie16.html) ersetzen). Ich habe auch schon testweise den Player-Sprite eine Animation verpasst.
