...
Die Idee zum heutigen Beispiel habe ich schamlos bei *[David Bouchard](https://www.youtube.com/@davidbouchard)* geklaut, der in seiner Einführung in [P5.play](http://cognitiones.kantel-chaos-team.de/multimedia/spieleprogrammierung/p5play.html) (einer Spielebibliothek zu [P5.js](http://cognitiones.kantel-chaos-team.de/programmierung/creativecoding/processing/p5js.html)) in den beiden Videos [Sprites Animation](https://www.youtube.com/watch?v=eDpTtxFhP2s) und [Game States](https://www.youtube.com/watch?v=6Vqpf_vLsH0) den Spieler in eine Zombie-Apcalypse schickt, in der er möglichst lange den Zombies ausweichen muß.

Für meine Version dieses Spiels hatte ich mir die freien ([CC0](https://itch.io/game-assets/assets-cc0)) Sprites und Tiles des [Dungein Tileset II](https://0x72.itch.io/dungeontileset-ii) des Users [0x72](https://0x72.itch.io/) ausgesucht, denn dort gab es nicht nur Zombies, sondern auch Halloween-Kürbisse als animierte Sprites. Und so wurde aus der *Zombie Apokalypse* kurzerhand die **Pumpkin Apokalypse**. Das Spieleprinzip ist jedoch (wenn auch stark vereinfacht) das gleiche geblieben:

Der Spieler kann mit den Pfeiltasten in alle vier Himmelsrichtungen bewegt werden und muß den bösen Halloween-Kürbissen ausweichen, von denen alle drei Sekunden ein neuer das Spielfeld betritt. Die Kürbisse bewegen sich zwar etwas langsamer als der Spieler und können sich auch nur horizontal in einer Richtung bewegen, da sie jedoch immer mehr werden, ist eine Kollision irgendwann unvermeidlich. Sobald der Spieler mit einem Kürbis kollidiert, beginnt das Spiel von vorne.
...
Daher ist es nun Zeit, die Kürbisse einzuführen. Auch sie sind eine eigene Klasse,
...
Dafür habe ich in `init()` mit `timer = 0` erst einmal einen Timer erzeugt, den ich in `update()` hochzähle. Alle drei Sekunden (`timer == 180`) erstelle ich dann einen neuen Pumpkin und setze (un der Funktion `createPumpkin()`) den Timer zurück:
...
Das Zeichnen der einzelnen Kürbisse ist dank der `show()`-Methode der Klasse `Pumkin` wieder sehr einfach:
